...
#   if taxon.parent_name and parent_rank:
#     query = """
#     MATCH (parent:Taxon)-[:PARENT_OF]->(tx: Taxon {name: $name, rank: $rank})
#     RETURN parent.name AS parent_name
#     """

#     existing = await db.query(query, name=taxon.name, rank=taxon.rank)

#     if existing and existing[0]["parent_name"] != taxon.parent_name:
#       raise ValueError(
#         f"Taxon '{taxon.name}' is already belong to '{existing[0]['parent_name']}'"
#       )
#   print("Check :")
#   query = """
...
#   WITH tx
#   WHERE $parent_name IS NOT NULL AND $parent_rank IS NOT NULL
...
#     parent_rank=parent_rank if parent_rank else None
#   )
...
async def merge_taxon(taxon: TaxonCreate) -> Taxon:
    ...
    # 2. Создаем/находим сам таксон и опционально его родителя
    query = """
    MERGE (tx:Taxon {name: $name, rank: $rank})
    ON CREATE SET tx.id = $tx_id
    
    WITH tx
    // Этот блок сработает ТОЛЬКО если есть и имя родителя, и его ранг
    ...
    if not result:
        # Если мы здесь, значит Neo4j вообще не смог выполнить MERGE (редко, но бывает)
